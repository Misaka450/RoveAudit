...
CREATE TABLE IF NOT EXISTS `sys_report` (
  ...
  `sql_content` MEDIUMTEXT NOT NULL COMMENT '查询SQL语句',
  `query_params` TEXT DEFAULT NULL COMMENT '查询参数配置（JSON格式）',
  `enable_download` TINYINT NOT NULL DEFAULT 1 COMMENT '是否允许下载：1-允许 0-禁止',
  `enable_chart` TINYINT NOT NULL DEFAULT 0 COMMENT '是否支持图表：1-支持 0-不支持',
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '状态：1-启用 0-禁用',
  `sort_order` INT NOT NULL DEFAULT 0 COMMENT '排序号',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='清单配置表';
...
CREATE TABLE IF NOT EXISTS `sys_warning_rule` (
  ...
  `sql_content` MEDIUMTEXT NOT NULL COMMENT '异常检测SQL',
  `risk_level` VARCHAR(20) NOT NULL DEFAULT 'low' COMMENT '风险等级：high/medium/low',
  `enable_flag` TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用：1-启用 0-禁用',
  `last_run_time` DATETIME DEFAULT NULL COMMENT '最后一次执行时间',
  `last_result_count` INT NOT NULL DEFAULT 0 COMMENT '最后一次检测出的异常数量',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='异常规则表';
...
CREATE TABLE IF NOT EXISTS `sys_download_log` (
  ...
  `user_id` INT NOT NULL COMMENT '下载用户ID',
  `username` VARCHAR(50) NOT NULL COMMENT '下载用户账号',
  `report_id` INT NOT NULL COMMENT '下载的清单ID',
  ...
  `data_count` INT NOT NULL DEFAULT 0 COMMENT '下载数据条数',
