...
export class WarningRule {
  ...
  @Column({ name: 'sql_content', type: 'mediumtext', comment: '异常检测 SQL 语句' })
  sqlContent: string;

  @Column({ name: 'risk_level', length: 20, default: 'low', comment: '风险等级：high/medium/low' })
  riskLevel: string;

  @Column({ name: 'enable_flag', type: 'tinyint', default: 1, comment: '是否启用：1-启用 0-禁用' })
  enableFlag: number;

  @Column({ name: 'last_run_time', type: 'datetime', nullable: true, comment: '最后一次执行时间' })
  lastRunTime: Date;
